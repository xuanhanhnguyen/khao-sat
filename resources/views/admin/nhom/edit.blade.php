...
@section('content_header')
    <h1>Sửa thông tin nhóm khảo sát mới</h1>
@stop
...
@section('content')
    ...
    <div class="callout-top callout-top-danger col-md-12">
        <form action="{{route('nhom.update', $data->id)}}" method="post">
            @method('put')
            {{ csrf_field() }}
            {{--name--}}
                    ...
                    <div class="form-check col-md-4">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="limit[]" id=""
                                   value="Sinh viên" {{array_search('Sinh viên', explode(',', $data->limit)) !== false ? 'checked':''}}>
                            Sinh viên
                        </label>
                    </div>
                    <div class="form-check col-md-4">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="limit[]" id=""
                                   value="Giảng viên" {{array_search('Giảng viên', explode(',', $data->limit)) !== false ? 'checked':''}}>
                            Giảng viên
                        </label>
                    </div>
                    <div class="form-check col-md-4">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="limit[]" value="Doanh nghiệp"
                                    {{array_search('Doanh nghiệp', explode(',', $data->limit)) !== false ? 'checked':''}}>
                            Doanh nghiệp
                        </label>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    <option @if($data->status == 1) selected @endif value="1">Công khai</option>
                    <option @if($data->status == 0) selected @endif value="0">Nhóm kín</option>
                </select>
